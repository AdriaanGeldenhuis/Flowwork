<?php
// /finances/reports/trial_balance.php
// Standalone, printable Trial Balance. Renders the SAME data as the in-page
// Trial Balance on /finances/reports.php by calling the canonical AJAX
// endpoint (report_trial_balance.php), so the numbers always agree.
...
<body class="fw-finance">
    <div class="fw-finance__container">
        <?php $finTitle = 'Trial Balance'; $finBack = '/finances/reports.php'; $finCompanyName = $companyName; $finFirstName = $firstName; include __DIR__ . '/../partials/header.php'; ?>
        ...
        <footer class="fw-finance__footer">
            <span>Trial Balance v<?= ASSET_VERSION ?> <span id="reportInfo" style="margin-left: 1rem;"></span></span>
    ...
    (function () {
        ...
        async function run() {
            ...
            var result = await FinanceAPI.request('/finances/ajax/report_trial_balance.php?date=' + encodeURIComponent(date));
            if (result && result.ok) {
                reportData = result.data;
                content.innerHTML = FWReport.trialBalance(reportData);
        ...
        exportBtn.addEventListener('click', function () {
            ...
            FWReport.download('trial-balance-' + reportDate.value + '.csv', FWReport.csv.trialBalance(reportData));
        });

        // Auto-run for today's date so the tab lands on a populated report.
        run();
    })();
    </script>
</body>
