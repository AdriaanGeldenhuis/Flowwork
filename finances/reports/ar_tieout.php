...
// Accounts Receivable tie-out: compares the AR control account balance in the
// general ledger against the AR subledger (open invoices − receipts − credit
// notes − write-offs) as at a date. A non-zero difference flags a control that
// has drifted from its subledger — a core SARS/audit reconciliation.
...
$gl   = $tie->glBalance('AR', $asOf);
$sub  = $tie->arSubledger($asOf);
$diff = round($gl - $sub, 2);
...
<head>
    ...
    <title>AR Tie-out – <?= htmlspecialchars($companyName) ?></title>
...
<body class="fw-finance">
    <div class="fw-finance__container">
        <?php $finTitle = 'AR Tie-out'; $finBack = '/finances/reports.php'; $finCompanyName = $companyName; $finFirstName = $firstName; include __DIR__ . '/../partials/header.php'; ?>
        <main class="fw-finance__main">
            ...
            <div class="fw-finance__report-content">
                <div class="fw-finance__report-header">
                    ...
                    <?php endif; ?>
                    <h2 class="fw-finance__report-subtitle">Accounts Receivable Tie-out</h2>
                ...
                <table class="fw-finance__report-table">
                    <tbody>
                        <tr>
                            <td>GL AR control balance</td>
                            <td class="fw-finance__report-table-number"><?= zar($gl) ?></td>
                        </tr>
                        <tr>
                            <td>AR subledger (invoices − receipts − credit notes − write-offs)</td>
                ...
                <?php if ($inBalance): ?>
                    <div class="fw-finance__alert fw-finance__alert--success" style="margin-top: 1rem;">AR control ties out to the subledger.</div>
                <?php else: ?>
                    <div class="fw-finance__alert fw-finance__alert--danger" style="margin-top: 1rem;">AR is OUT by <?= zar(abs($diff)) ?> — the GL control and subledger do not agree.</div>
                <?php endif; ?>

                <p class="fw-finance__report-prepared" style="margin-top: 1rem;">
                    GL AR uses the mapped AR control account (company settings, falling back to the seeded chart).
                    Subledger AR mirrors what the posting engine puts into the control: issued invoices, less allocated
                    and unallocated receipts, credit notes and write-offs, in ZAR at each document's captured rate.
                </p>
            </div>
        </main>
        <footer class="fw-finance__footer">
            <span>AR Tie-out v<?= ASSET_VERSION ?></span>
